...
// \`\`0\`\` or \`\`WGDEVICE\_F\_REPLACE\_PEERS\`\` if all current peers should be
...
// The index/type parameter is unused on \`\`SET\_DEVICE\`\` operations and is
...
for entry in attrs.get_peers() {
  ...
  // \`\`0\`\` and/or \`\`WGPEER\_F\_REMOVE\_ME\`\` if the specified peer should not
  ...
  // struct sockaddr\_in or struct sockaddr\_in6
  ...
  for entry in entry.get_allowedips() {
    ...
    // IP family, either \`\`AF\_INET\`\` or \`\`AF\_INET6\`\`\.
    entry.get_family(); // u16

    // Either \`\`struct in\_addr\`\` or \`\`struct in6\_addr\`\`\.
    entry.get_ipaddr(); // IpAddr
    entry.get_cidr_mask(); // u8

    // \`\`WGALLOWEDIP\_F\_REMOVE\_ME\`\` if the specified IP should be removed;
